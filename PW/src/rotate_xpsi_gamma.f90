...
!
! >>> This file is copied from rotate_wfc_gamma.f90,
! >>> and modified by Satomichi Nishihara
!
!----------------------------------------------------------------------------
SUBROUTINE rotate_xpsi_gamma( npwx, npw, nstart, gstart, nbnd, psi, overlap, evc, hevc, sevc, e )
  !----------------------------------------------------------------------------
  !
  ...
  IF ( overlap ) THEN 
     ...
     !
     CALL DGEMM( 'T', 'N', nstart, nstart, 2 * npw, 2.D0 , psi,  2 * npwx, spsi, 2 * npwx, 0.D0, sr, nstart )
     !            
     IF ( gstart == 2 ) &
  ...
  !
  IF ( overlap ) &
  CALL DGEMM( 'N', 'N', 2 * npw, nbnd, nstart, 1.D0, spsi, 2 * npwx,  vr, nstart, 0.D0, sevc, 2 * npwx )
  !
  DEALLOCATE( en )
  DEALLOCATE( vr )
  DEALLOCATE( sr )
...
!
!----------------------------------------------------------------------------
SUBROUTINE protate_xpsi_gamma( npwx, npw, nstart, gstart, nbnd, psi, overlap, evc, hevc, sevc, e )
  !----------------------------------------------------------------------------
  !
        ...
        IF( ic <= nbnd ) THEN
           ...
           DO ipr = 1, desc%npr
              ...
              IF( ipr-1 == desc%myr .AND. ipc-1 == desc%myc .AND. la_proc ) THEN
                 ...
                 IF ( overlap ) &
                 CALL DGEMM( 'N', 'N', 2*npw, nc, nr, 1.D0,  &
                             spsi(1,ir), 2*npwx, vr, nx, beta, sevc(1,ic), 2*npwx )
                 !
              ELSE
                 !
                 ...
                 IF ( overlap ) &
                 CALL DGEMM( 'N', 'N', 2*npw, nc, nr, 1.D0,  &
                             spsi(1,ir), 2*npwx, vtmp, nx, beta, sevc(1,ic), 2*npwx )
                 !
              END IF
              ! 
